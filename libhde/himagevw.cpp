...
HImageView::HImageView(QWidget *parent)
  : QWidget(parent)
{
  initUi();
  imageLabel->setText("Generating Preview...");
  leftsideHidden = false;
  buildDirTree();

  closeBtn->setShortcut(QKeySequence(Qt::Key_Escape));
  connect( splitcloseBtn, SIGNAL(clicked()), this, SLOT(closeLeftSide()) );
  connect( resizesplitBtn, SIGNAL(clicked()), this, SLOT(resizeLeftSide()) );

  connect( magplusBtn, SIGNAL(clicked()), this, SLOT(magplusClicked()) );
  connect( magminBtn, SIGNAL(clicked()), this, SLOT(magminClicked()) );

  connect( rotatecclockBtn, SIGNAL(clicked()), this, SLOT(rotatecclockClicked()) );
  connect( rotateclickBtn, SIGNAL(clicked()), this, SLOT(rotateclickClicked()) );
  connect( aboutBtn, SIGNAL(clicked()), this, SLOT(aboutClicked()) );

}

void HImageView::initUi()
{
...
/*
  ...
  */
  ...
  vboxLayout1->addItem(spacerItem);
    ...
    bestfitBtn = new QPushButton(this);
    ...
    actualsizeBtn = new QPushButton(this);
    ...
    magplusBtn = new QPushButton(this);
    ...
    magminBtn = new QPushButton(this);
    ...
    rotatecclockBtn = new QPushButton(this);
    ...
    rotateclickBtn = new QPushButton(this);
...
void HImageView::retranslateUi()
{
    setWindowTitle(QApplication::translate("HImageView", "Haydar Image Viewer", 0, QApplication::UnicodeUTF8));
...
void HImageView::buildDirTree()
{
  model = new QDirModel;
  model->setFilter(QDir::AllDirs|QDir::NoDotAndDotDot);
  model->setReadOnly( true );
  dirTree->setModel(model);
//  dirTree->setRootIsDecorated( false );
  dirTree->hideColumn ( 1 );
  dirTree->hideColumn ( 2 );
  dirTree->hideColumn ( 3 );
  dirTree->hideColumn ( 4 );
  dirTree->resizeColumnToContents ( 0 );

  connect( dirTree, SIGNAL(activated ( const QModelIndex & )), this,
           SLOT( dirActivated ( const QModelIndex & ) ) );

  connect( dirTree, SIGNAL( clicked( const QModelIndex & )), this,
           SLOT( dirActivated ( const QModelIndex & ) ) );

  connect( prevBtn, SIGNAL( clicked() ), this, SLOT( prevClicked() ));
  connect( nextBtn, SIGNAL( clicked() ), this, SLOT( nextClicked() ));
}
 
void HImageView::load( QString file)
{
  imageLabel->setText("Generating Preview...");
  QFileInfo fileInfo(file);
  dirTree->setCurrentIndex( model->index( fileInfo.absolutePath() ) );
  dirActivated( model->index( fileInfo.absolutePath() ) );
  dirTree->resizeColumnToContents ( 0 );
  
  for(int i=0; i < imagesList->count(); i++)
  {
    QListWidgetItem * itm = imagesList->item( i );
    if( itm->text() == fileInfo.fileName() )
      imagesList->setCurrentRow( i );
  }
}



void HImageView::closeLeftSide()
{
  leftFrame->close();
  //splitterFrame->close();
}

void HImageView::resizeLeftSide()
{
  if(leftsideHidden)
  {
    splitter->show();
    leftsideHidden = false;
    resizesplitBtn->setText("<");
  }
  else
  {
    splitter->hide();
...
void HImageView::dirActivated( const QModelIndex & index )
{
  //dirTree->setCurrentIndex( model->index( QFileInfo(file).absolutePath() ) );
  disconnect( imagesList, SIGNAL( currentItemChanged( QListWidgetItem *, QListWidgetItem *) ),
           this, SLOT( currentFileChanged( QListWidgetItem *, QListWidgetItem *)) ); 
  imagesList->clear();
  QString path;
  ...
  for (int i = 0; i < list.size(); ++i)
  {
    ...
    if( fileInfo.isFile() && fileInfo.isReadable() ) 
    {
      ...
      if(!pix.isNull())
        new QListWidgetItem( QIcon(pix), fileInfo.fileName(), imagesList);
    }
  }
  connect( imagesList, SIGNAL( currentItemChanged( QListWidgetItem *, QListWidgetItem *) ),
           this, SLOT( currentFileChanged( QListWidgetItem *, QListWidgetItem *)) ); 

}

void HImageView::currentFileChanged ( QListWidgetItem * current, QListWidgetItem * previous )
{
  imageLabel->setText("Generating Preview...");
  QString file = curDir + "/" + current->text();
  
  QPixmap pix(file);
  if( (pix.width() > imageLabel->width()) || ( pix.height() > imageLabel->height() ) )
    pix = pix.scaled(imageLabel->size(), Qt::KeepAspectRatio, Qt::SmoothTransformation);
  imageLabel->setPixmap(pix);
}

void HImageView::prevClicked()
{
  imageLabel->setText("Generating Preview...");
  if( imagesList->currentRow() > 0 )
    imagesList->setCurrentRow( imagesList->currentRow()-1 );
  else
    imagesList->setCurrentRow( imagesList->count()-1 );
}

void HImageView::nextClicked()
{
  imageLabel->setText("Generating Preview...");
  if( imagesList->currentRow() < imagesList->count()-1)
    imagesList->setCurrentRow( imagesList->currentRow()+1 );
  else
    imagesList->setCurrentRow( 0 );  
}

void HImageView::magplusClicked()
{
  QString file = curDir + "/" + imagesList->currentItem()->text();
  
  QPixmap pix(file);
  pix = pix.scaled(imageLabel->pixmap()->size()*1.2, Qt::KeepAspectRatio, Qt::SmoothTransformation);
  imageLabel->setPixmap(pix);
}

void HImageView::magminClicked()
{
  QString file = curDir + "/" + imagesList->currentItem()->text();
...
void HImageView::rotatecclockClicked()
{
...
void HImageView::rotateclickClicked()
{
  QPixmap pix= *imageLabel->pixmap();
  QMatrix matrix;
  //matrix.translate(50, 50);
...
void HImageView::aboutClicked()
{
  QMessageBox::information(this, tr("Haydar Image Viewer"), 
                                 tr("Haydar Image Viewer: This program is created to\n"
                                    "be used for viewing all types of images supported\n"
                                    "by Qt 4.3.0 and above.\n\n"
                                    "Created by: Haydar Alkaduhimi\n"
                                    "Under GPL License"));
}
