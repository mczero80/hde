...
pager::pager(QWidget *parent) : QLabel(parent)
{
  ...
  setFixedSize((int)((defaults::vdesks*defaults::pager_height
		      *(float)dwidth)/(float)dheight)+2,
	       defaults::pager_height);
...
void pager::draw_pager(void)   // redraw pager and all clients
{
  ...
  backpix->fill(QColor(238,234,238));
  ...
  for(i = 0; i < nwins; i++)
  {
    ...
    if((client = qapp::pwindows[wins[i]]) == NULL || ! client->isVisible() 
       || client->is_unmapped())
      continue;
    
    // adjust for virtual desktop
    crect.setRect(client->x()+(qapp::adesk*dwidth), client->y(),
...
{
  QColor fcol = (crect.x()/dwidth == qapp::adesk || crect.right()/dwidth 
		 == qapp::adesk)?(defaults::pager_visible):
    (defaults::pager_window);
...
void pager::change_desk(int desk)  // change virtual desktop
{
  ...
  foreach(client, clients)
  {
    ...
    if(! (client->get_pflags() & qapp::Sticky) && (client->isVisible() ||
						   ! curdesk))
    {
